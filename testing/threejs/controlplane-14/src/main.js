...
const envStudio = (() => {
  ...
  softLight('White overhead', new THREE.Color(5.5, 5.6, 6.0),    12, 12,  new THREE.Vector3(-3, 8, -1));   // the silvery "shine"
...
const post = (() => {
  ...
  function blurInto(src, a, b, texel) {
  ...
  function render() {
    ...
    if (C.dof.enabled) {
      blurInto(hdr.texture, dofA, dofB, halfTexel);
...
// ---------------------------------------------------------------- resolution
// Two budgets. Live: the adaptive pixel ratio (steps down while frames run
// long, never back up). Still: while motion is frozen the frame is rendered
// once at native ratio x supersample, capped by a pixel budget, and only
// re-rendered when something changes - so a frozen frame is print-quality
// and costs nothing per second.
const Q = C.quality;
...
function adaptResolution(now) {
  ...
  if (avg > Q.targetFrameMs && livePixelRatio > Q.minPixelRatio) {
    livePixelRatio = Math.max(Q.minPixelRatio, +(livePixelRatio - 0.25).toFixed(2));
    applySize();
  }
}
...
function tick() {
  ...
  if (paused !== stillMode) {
    ...
    frameAccum = 0; frameCount = 0; lastFrame = performance.now();
  }
  ...
  if (needsRender) {
    ...
    post.render();
    needsRender = false;
    if (!paused) adaptResolution(performance.now());
  }
