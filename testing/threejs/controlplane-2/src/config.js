...
export const CONFIG = {
  ...
  // where the orb sits at each end of the scroll
  orbPath: {
    startY: -4.1,          // corrupted: low in frame, cut by the bottom edge
    startScaleXZ: 1.2,     // corrupted: spread wide like a mound
    startScaleY: 1.0,
    endXFrac: 0.45,        // repaired: this fraction of the visible half-width, pushed left
    endXMax: 3.0,          // ...but never further than this (world units)
    uiFadeOut: [0.05, 0.35],  // scroll range over which the hero fades away
    uiFadeIn: [0.72, 0.95],   // scroll range over which the reveal block fades in
  },
